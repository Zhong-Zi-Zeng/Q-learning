...
"""建立神經網路"""
def build_dqn(lr,n_actions,input_shape,use_dueling):
    ...
    if use_dueling:
        ...
        Q = Dense(n_actions,activation='linear')(h4)
...
class Agent():
    def __init__(self
        ...
        """建置資料庫"""
        self.memory = ReplayBuffer(max_mem=mem_size,n_action=n_actions,input_shape=self.input_shape)
        """建置模型"""
        self.q_eval = build_dqn(lr=self.alpha,n_actions=self.n_actions,input_shape=self.input_shape,use_dueling=use_dueling)
        self.q_target_net = build_dqn(lr=self.alpha, n_actions=self.n_actions,input_shape=self.input_shape,use_dueling=use_dueling)
