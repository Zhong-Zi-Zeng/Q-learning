...
class Agent():
    ...
    def learn(self):
        ...
        if self.use_ddqn:
            q_eval_next_state = np.argmax(self.q_eval.predict(next_state), axis=1)
            q_target[batch_index, action_indices] = reward + self.gamma * q_target_pre[batch_index, q_eval_next_state] * done
        else:
            # 貝爾曼方程
